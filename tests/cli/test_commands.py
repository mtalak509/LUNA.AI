...
from core.agent.workspace import list_attachments


@pytest.fixture
def cli_zone(tmp_path, monkeypatch):
    """Point the CLI at a throwaway session. Returns (attachments dir, checkpoints)."""
    session_dir = tmp_path / "session"
    (session_dir / "workspace").mkdir(parents=True)
    monkeypatch.setattr(commands, "SESSION_DIR", session_dir)
    return session_dir / "workspace" / "attachments", session_dir / "checkpoints"


def test_attach_copies_file_and_snapshots(cli_zone, tmp_path, capsys):
    attachments, checkpoints = cli_zone
    ...
    assert "прикреплено: attachments/report.json" in out
    # Same guarantee as the route: /undo after an attach must not swallow the file.
    (snapshot,) = list(checkpoints.iterdir())
    assert (snapshot / "attachments" / "report.json").is_file()
    assert snapshot.name in out


def test_attach_lands_where_the_agent_looks(cli_zone, tmp_path):
    """Parity with the HTTP upload: the REPL puts the file in the same one directory."""
    attachments, _ = cli_zone
    source = tmp_path / "report.json"
    source.write_bytes(b"{}")

    commands._attach(str(source))

    (listed,) = list_attachments(attachments.parent)
    assert listed.path == "report.json"
...
def test_attach_handles_paths_with_spaces_and_quotes(cli_zone, tmp_path, capsys):
    ...
    assert (attachments / "my big report.json").is_file()
    assert "прикреплено: attachments/my big report.json" in capsys.readouterr().out
...
def test_attach_refuses_duplicate_until_overwrite(cli_zone, tmp_path, capsys):
    ...
    assert (attachments / "report.json").read_bytes() == b"first"
...
def test_attach_is_dispatched_by_handle_command(cli_zone, tmp_path, capsys):
    ...
    assert "прикреплено" in capsys.readouterr().out
